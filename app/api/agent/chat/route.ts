...
export async function POST(req: Request) {
  try {
    ...
    const prompt =
      body.prompt && typeof body.prompt === "string" ? body.prompt : undefined;
    const incoming = Array.isArray(body.messages)
      ? body.messages
      : prompt
      ? ([{ role: "user", content: prompt }] as ClientMessage[])
      : [];
    if (!incoming.length)
      return NextResponse.json(
        { ok: false, error: "No prompt or messages provided" },
        { status: 400 }
      );
    ...
    const config = {
      configurable: { thread_id: body.threadId || `web_${Date.now()}` },
    };
    ...
    const ruleIntent =
      /(suggest|create|setup|build|make|generate|recommend).*?(rule|auto.*pilot|strategy|dca|rebalance)/i.test(
        lastUserMsg
      );

    // Address query intent (bypass agent for instant response)
    const addressIntent =
      /\b(what.{0,20}(is|s).{0,20}(my|the).{0,20}(address|wallet)|my.{0,10}address|my.{0,10}wallet|show.{0,10}address|get.{0,10}address)\b/i.test(
        lastUserMsg
    ...
    if (addressIntent) {
      try {
        ...
        const clientEOA =
          body.walletAddress && /^0x[a-fA-F0-9]{40}$/.test(body.walletAddress)
            ? body.walletAddress
            : undefined;
        ...
        response +=
          "💡 *Use the Smart Account address for gasless transactions and receiving funds.*";
        ...
        return NextResponse.json({
          ok: true,
          content: `❌ Failed to retrieve addresses: ${
            e?.message || String(e)
          }`,
          threadId: config.configurable.thread_id,
        });
    ...
    // Balance query intent (bypass agent for instant response)
    // More flexible regex that handles typos like "ballance"
    const balanceIntent =
      /\b(what.{0,20}(is|s).{0,20}(my|the).{0,20}bala?l?ance|my.{0,10}bala?l?ance|show.{0,10}bala?l?ance|get.{0,10}bala?l?ance|check.{0,10}bala?l?ance|bala?l?ances?)\b/i.test(
        lastUserMsg
      );
    console.log("💰 [BALANCE] Intent detected:", balanceIntent, "for message:", lastUserMsg);
    if (balanceIntent) {
      try {
        const { getEOAAddress, getBalance, getChainInfo } = await getAgent(
          chainOverride
        );
        
        // Use EOA address instead of smart account for Celo testnet
        const addr = await getEOAAddress();
        console.log("💰 [BALANCE] Using EOA address:", addr);
        
        const chainInfo = getChainInfo();
        const chainId = chainInfo.chainId;
        const nativeSymbol = chainInfo.nativeSymbol;
        
        console.log("💰 [BALANCE] Chain info:", {
          chainId,
        ...
        const tokenMatch = lastUserMsg.match(
          /\b(USDC|USDT|DAI|WETH|WBTC|WAVAX|CUSD|CEUR|ETH|BTC|AVAX|CELO)\b/i
        );
        const tokenSymbol = tokenMatch?.[1]?.toUpperCase();
        console.log("💰 [BALANCE] Token filter:", tokenSymbol || "all tokens");

        if (tokenSymbol && tokenSymbol !== nativeSymbol) {
          // Get specific token balance
          console.log(`💰 [BALANCE] Querying specific token: ${tokenSymbol}`);
          const token = resolveTokenBySymbol(tokenSymbol, chainId);
          
          if (!token) {
            console.log(`❌ [BALANCE] Token ${tokenSymbol} not found on chain ${chainId}`);
            return NextResponse.json({
              ok: true,
          ...
          console.log(`💰 [BALANCE] Resolved token:`, {
            symbol: token.symbol,
          ...
          const formattedBal =
            balNum >= 1 ? balNum.toFixed(4) : balNum.toExponential(4);

          console.log(`💰 [BALANCE] Token balance for ${tokenSymbol}:`, balance);
          ...
          console.log(`💰 [BALANCE] Querying native token balance for address: ${addr}`);
          ...
          console.log(`💰 [BALANCE] Native balance (${nativeSymbol}):`, nativeBalance);

          let response = `💰 **Your Balance:**\n\n`;
          response += `**Native Token (${nativeSymbol}):**\n\`${formattedBal} ${nativeSymbol}\`\n\n`;
          response += `📍 EOA Account: \`${addr}\`\n`;
          response += `🌐 Network: ${chainInfo.chainName}`;

          // Check for token balances (cUSD, USDC, etc.)
          const commonTokens = ["CUSD", "USDC", "USDT"];
          const tokenBalances: string[] = [];
          
          console.log(`💰 [BALANCE] Checking common tokens:`, commonTokens);

          // For Celo Sepolia, check all tokens from our updated token list
          if (chainId === 11142220) {
            console.log(`💰 [BALANCE] Checking tokens for Celo Sepolia...`);
            
            // Define Celo Sepolia token addresses directly
            const celoTokens = [
              { symbol: 'cUSD', address: '0x874069Fa1Eb16D44d622F2e0Ca25eeA172369bC1', decimals: 18 },
              { symbol: 'USDC', address: '0x98Cc94F9271e36EB1c30d77c39dCb9929D9310C4', decimals: 6 },
              { symbol: 'USDT', address: '0x48065fbBE25f71C9282ddf5e1cD6D6A887483D5e', decimals: 6 },
            ];
            
            for (const token of celoTokens) {
              try {
                console.log(`💰 [BALANCE] Checking ${token.symbol} at ${token.address}...`);
                ...
                const balNum = Number(balanceRaw) / (10 ** token.decimals);
                console.log(`💰 [BALANCE] ${token.symbol} balance (raw: ${balanceRaw}):`, balNum);
                
                if (balNum > 0) {
                  tokenBalances.push(`• ${token.symbol}: \`${balNum.toFixed(6)}\``);
                } else if (balNum === 0) {
                  console.log(`💰 [BALANCE] ${token.symbol} balance is exactly 0`);
                }
              } catch (tokenErr: any) {
                console.error(`❌ [BALANCE] Error checking ${token.symbol}:`, tokenErr?.shortMessage || tokenErr?.message || tokenErr);
              }
            }
            
            // Check recent swap transaction to debug
            console.log(`💰 [BALANCE] Checking recent swap transaction for debugging...`);
            try {
              const recentTxHash = '0x3389fa7b2c8f6316f7667bb0e2426d832896430461904dd096c7e96d56ecd40e' as `0x${string}`;
              const receipt = await publicClient.getTransactionReceipt({ hash: recentTxHash });
              console.log(`💰 [BALANCE] Transaction status:`, receipt.status);
              console.log(`💰 [BALANCE] Transaction logs count:`, receipt.logs.length);
              
              // Look for Transfer events to see where tokens went
              for (const log of receipt.logs) {
                if (log.topics[0] === '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef') {
                  // This is a Transfer event
                  const to = '0x' + log.topics[2]?.slice(26);
                  console.log(`💰 [BALANCE] Transfer event found - token: ${log.address}, to: ${to}`);
                }
              }
            } catch (txErr: any) {
              console.error(`❌ [BALANCE] Could not check transaction:`, txErr?.message);
            }
          } else {
            // For non-Celo chains, check standard tokens
            for (const tokenSym of commonTokens) {
              try {
                const token = resolveTokenBySymbol(tokenSym, chainId);
                if (token && token.address !== "CELO" && token.address !== "AVAX") {
                  console.log(`💰 [BALANCE] Checking ${tokenSym} at ${token.address}...`);
                  const bal = await getBalance(token.address as Address, addr);
                  const balNum = parseFloat(bal);
                  console.log(`💰 [BALANCE] ${tokenSym} balance:`, bal);
                  
                  if (balNum > 0) {
                    tokenBalances.push(`• ${tokenSym}: \`${balNum.toFixed(4)}\``);
                  }
                } else {
                  console.log(`💰 [BALANCE] Skipping ${tokenSym} (not found or native token)`);
                }
              } catch (e: any) {
                console.error(`❌ [BALANCE] Error checking ${tokenSym}:`, e?.message || e);
                // Skip if token check fails
              }
            }
          }
          ...
          if (tokenBalances.length > 0) {
            response += `\n\n**ERC-20 Tokens:**\n${tokenBalances.join("\n")}`;
            console.log(`💰 [BALANCE] Found ${tokenBalances.length} tokens with balance`);
          } else {
            console.log(`💰 [BALANCE] No ERC-20 tokens with balance found`);
          }

          if (balNum === 0 && tokenBalances.length === 0) {
            response +=
          ...
          console.log(`✅ [BALANCE] Balance check complete`);
          return NextResponse.json({
            ok: true,
            content: response,
            threadId: config.configurable.thread_id,
    ...
    const mcpIntent =
      /(analy[sz]e|analysis|prediction|predict|forecast|strategy|strategies|portfolio\s+strategy|chart|charts|graph|graphs)/i.test(
        lastUserMsg
      );
    ...
    const mcpCoinMatch =
      lastUserMsg.match(
        /(?:analy[sz]e|predict|forecast|strategy|chart)\s+([a-z0-9\-]{2,40})/i
      ) || // "analyze bitcoin"
      lastUserMsg.match(/(?:of|for|on|about)\s+([a-z0-9\-]{2,40})/i) || // "analysis of bitcoin"
      lastUserMsg.match(
        /\b([A-Za-z]{2,10})\b\s+(?:analysis|forecast|prediction|strategy)/i
      ); // "bitcoin analysis"
    ...
    if (mcpIntent && mcpCoinMatch && isValidCoin) {
      try {
        ...
        const horizonMatch = lastUserMsg.match(
          /(?:next|over|for)\s+(\d{1,3})\s*(?:days?|d)/i
        );
        const horizonDays = horizonMatch
          ? Math.min(365, Math.max(1, parseInt(horizonMatch[1], 10)))
          : 30;
        const granularity: "1h" | "4h" | "1d" = /\b1h\b/i.test(lastUserMsg)
          ? "1h"
          : /\b4h\b/i.test(lastUserMsg)
          ? "4h"
          : "1d";
        ...
        else if (/\b(candlestick|candle|ohlc)\b/i.test(lastUserMsg))
          chartType = "candlestick";
        else if (/\b(area|area\s+chart)\b/i.test(lastUserMsg))
          chartType = "area";
        else if (
          /\b(line|linear|line\s+chart|linear\s+graph)\b/i.test(lastUserMsg)
        )
          chartType = "line";

        // Fetch historical data for context (1 year by default)
        const historicalContext = await fetchHistoricalData(
          coin,
          365,
          false
        ).catch(() => null);
        ...
        if (!resp.ok) {
          return NextResponse.json({
            ok: true,
            content: `❌ MCP error: ${resp.error || "Unknown error"}${
              health.ok ? "" : `\nHealth: ${health.message || "unreachable"}`
            }`,
            threadId: config.configurable.thread_id,
          });
        }
        ...
        if (historicalContext?.ok && historicalContext.statistics) {
          ...
          parts.push(
            `📊 **${coin.toUpperCase()} - 1 Year Overview**\n💰 Current: $${stats.currentPrice.toFixed(
              6
            )} | 📈 High: $${stats.highestPrice.toFixed(
              6
            )} | 📉 Low: $${stats.lowestPrice.toFixed(6)}\n📊 Change: ${
              stats.priceChangePercent >= 0 ? "+" : ""
            }${stats.priceChangePercent.toFixed(
              2
        ...
        if (resp.predictions?.length) {
          const next3 = resp.predictions
            .slice(0, 3)
            .map(
              (p) =>
                `• ${p.date}: $${Number(p.price).toFixed(4)}${
                  p.probability
        ...
        if (resp.strategies?.length) {
          const s = resp.strategies
            .slice(0, 2)
            .map((x) => `• ${x.name} (${x.risk}) — ${x.description}`)
            .join("\n");
        ...
        return NextResponse.json({
          ok: true,
          content: `❌ MCP request failed: ${e?.message || String(e)}`,
          threadId: config.configurable.thread_id,
        });
      }
    }
    ...
    if (ruleIntent && mcpCoinMatch && isValidCoin) {
      try {
        ...
        const baseUrl =
          process.env.MCP_ANALYTICS_URL || "http://localhost:8080";
        ...
        if (data.ok && data.suggestions?.length) {
          const parts: string[] = [];
          parts.push(
            `🤖 **Auto-Pilot Rule Suggestions for ${coin.toUpperCase()}**\n`
          );

          data.suggestions.forEach((sug: any, idx: number) => {
            parts.push(
              `\n**${idx + 1}. ${sug.strategy} Strategy** (${
                sug.riskLevel
              } risk)`
            );
            ...
            parts.push(
              `• Cooldown: ${sug.suggestedParams.cooldownMinutes} minutes`
            );
            if (sug.suggestedParams.rotateTopN) {
              parts.push(
                `• Rotate Top: ${sug.suggestedParams.rotateTopN} coins`
              );
            }
          });
    ...
    const historyIntent =
      /(history|historical|past\s+(?:price|data)|(?:price|data)\s+history|year\s+(?:old|data)|month\s+(?:old|data)|since)/i.test(
        lastUserMsg
      );
    const historyMatch =
      lastUserMsg.match(
        /(?:history|historical|past|data)\s+(?:of|for|on)?\s*([a-z0-9\-]{2,40})/i
      ) || // "history of bitcoin"
      lastUserMsg.match(
        /([a-z0-9\-]{2,40})\s+(?:history|historical|past\s+data|price\s+history)/i
      ) || // "bitcoin history"
      lastUserMsg.match(
        /(\d+)\s+(?:year|month|day)s?\s+(?:of|for)?\s*([a-z0-9\-]{2,40})/i
      ); // "1 year bitcoin"
    console.log(
    ...
    if (historyIntent && historyMatch) {
      try {
        ...
        if (!blacklistedWords.includes(coin)) {
          ...
          const maxMatch = /\b(all|max|maximum|complete|full|entire)\b/i.test(
            lastUserMsg
          );
          ...
          if (histData.ok && histData.statistics) {
            ...
            // Add trend analysis
            const trend =
              histData.statistics.priceChangePercent > 0
                ? "📈 Upward trend"
                : "📉 Downward trend";
            const volatilityLevel =
              histData.statistics.volatility > 10
                ? "High"
                : histData.statistics.volatility > 5
                ? "Medium"
                : "Low";

            const enhancedResponse = `${response}\n\n**Analysis:**\n${trend}\nVolatility Level: ${volatilityLevel}\nData Period: ${
              days === "max" ? "Maximum available" : `${days} days`
            ...
            return NextResponse.json({
              ok: true,
              content: `❌ Could not fetch historical data for ${coin}. ${
                histData.error || "Please check the coin name and try again."
              }`,
              threadId: config.configurable.thread_id,
            });
          }
    ...
    let topCoinsMatch = text.match(
      /top\s+(\d+)\s+(?:coin|crypto|cryptocurrency|token)/i
    );
    console.log("Top coins match:", topCoinsMatch);
    if (!topCoinsMatch)
      topCoinsMatch = text.match(
        /show\s+(?:me\s+)?(\d+)\s+(?:coin|crypto|cryptocurrency|token)/i
      );
    if (!topCoinsMatch)
      topCoinsMatch = text.match(
        /(\d+)\s+(?:top|best)\s+(?:coin|crypto|cryptocurrency)/i
      );

    if (
      topCoinsMatch ||
      /\b(top|ranking|market|cryptocurrencies)\b/i.test(text)
    ) {
            ...
            const coinsText = coins
              .map((coin: any, index: number) => {
                ...
                return `${index + 1}. **${coin.name} (${
                  coin.symbol
                })**\n   💰 ${formatPrice(coin.price)} | 📊 ${formatMarketCap(
                  coin.marketCap
    ...
    if (
      /\b(?:gas\s+(?:price|fee|cost)|avax\s+gas|current\s+gas|network\s+fee|gas\s+estimate)\b/i.test(
        text
      )
    ) {
      try {
        ...
        const rpcUrl =
          chainId === 43114
            ...
            : process.env.RPC_URL_FUJI ||
              process.env.NEXT_PUBLIC_RPC_URL_FUJI ||
        ...
        const networkName =
          chainId === 43114 ? "Avalanche Mainnet" : "Avalanche Fuji Testnet";

        return NextResponse.json({
          ok: true,
          content: `⛽ **${networkName} Gas Price**\n\nCurrent: ${gasPriceGwei.toFixed(
            2
    ...
    if (/\b(balance|balances)\b/.test(text)) {
      try {
        ...
        return NextResponse.json({
          ok: true,
          content: `💰 **Your Balance** (Avalanche ${
            chainId === 43114 ? "Mainnet" : "Fuji"
          })\n\nAddress: ${eoaAddress}\n\nAVAX: ${avaxFormatted.toFixed(
            4
          )}${tokenBalances}`,
          threadId: config.configurable.thread_id,
        });
    ...
    if (/\b(smart\s+(?:account\s+)?balance|smart\s+account)\b/i.test(text)) {
      try {
        ...
        return NextResponse.json({
          ok: true,
          content: `🏦 **Smart Account Balance** (Avalanche ${
            chainId === 43114 ? "Mainnet" : "Fuji"
        ...
        return NextResponse.json({
          ok: true,
          content: `❌ Failed to check smart account balance: ${
    ...
    if (
      /\b(smart\s+(?:account\s+)?address|smart\s+account)\b/i.test(text) &&
      !/balance/i.test(text)
    ) {
      try {
        const smartAddress = await getSmartAddressOrNull();
        ...
        return NextResponse.json({
          ok: true,
          content: `❌ Failed to get smart account address: ${
            error instanceof Error ? error.message : "Unknown error"
          }`,
          threadId: config.configurable.thread_id,
        });
    ...
    const transferRe =
      /transfer\s+(\d+(?:\.\d+)?)\s*(?:([A-Za-z]{2,6}))?\s*(?:tokens?)?\s*(?:to|=>)\s*(0x[a-fA-F0-9]{40})/;
      ...
      try {
        if (symbol.toUpperCase() === "AVAX") {
          ...
          if (currentBalance < totalCost) {
            ...
            return NextResponse.json({
              ok: true,
              content: `❌ **Insufficient Balance**\n\nCurrent Balance: ${currentBalanceEth.toFixed(
                6
              )} AVAX\nTransfer Amount: ${amount} AVAX\nEstimated Gas Cost: ${gasCostEth.toFixed(
                6
              )} AVAX\nTotal Needed: ${totalCostEth.toFixed(
                6
              )} AVAX\n\nYou need ${(totalCostEth - currentBalanceEth).toFixed(
                6
              )} more AVAX to complete this transfer.`,
              threadId: config.configurable.thread_id,
            });
          }
          ...
          return NextResponse.json({
            ok: true,
            content: `✅ Successfully transferred ${amount} AVAX to ${to.slice(
              0,
              8
            )}...${to.slice(-6)}\n\nTransaction: ${txHash}\nGas Used: ${
              Number(gasCost) / 1e18
    ...
    const priceMatch =
      text.match(
        /\b(?:price\s+of\s+|what'?s\s+|current\s+price\s+of\s+)?([a-z]{2,10})\s+(?:price|worth|value|cost)\b/i
      ) ||
    ...
    if (priceMatch && priceMatch[1]) {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const coin = data.data?.coins?.find(
            (c: any) =>
              c.symbol.toUpperCase() === sym ||
              c.name.toLowerCase() === sym.toLowerCase()
          );
    ...
    const swapRe =
      /swap\s+(\d+(?:\.\d+)?)\s*([A-Za-z]{2,10})\s*(?:to|for|->)\s*([A-Za-z]{2,10})/i;
    ...
    if (swMatch) {
      ...
      try {
        ...
        try {
          ...
          console.error(
            "❌ [SWAP] customSwap failed:",
            customErr?.message || customErr
          );
          ...
          try {
            ...
            try {
              ...
              console.log(`🔄 [SWAP] Resolved tokens:`, {
                in: inToken
                  ? `${inToken.symbol} (${inToken.address})`
                  : "NOT FOUND",
                out: outToken
                  ? `${outToken.symbol} (${outToken.address})`
                  : "NOT FOUND",
              });

              if (!inToken) {
                console.error(
                  `❌ [SWAP] Could not resolve input token: ${fromSym}`
                );
              }
              if (!outToken) {
                console.error(
                  `❌ [SWAP] Could not resolve output token: ${toSym}`
                );
              }
              ...
              const tokenInAddress =
                inToken &&
                inToken.address !== "AVAX" &&
                inToken.address !== "CELO"
                  ? (inToken.address as Address)
                  : undefined;
              const tokenOutAddress =
                outToken &&
                outToken.address !== "AVAX" &&
                outToken.address !== "CELO"
              ...
              console.log(
                "✅ [SWAP] smartSwap with resolved tokens succeeded:",
                resolvedResult.hash
              );
              return NextResponse.json({
                ok: true,
              ...
              console.error(
                "❌ [SWAP] Token resolution and retry failed:",
    ...
    console.log(
      "No specific intent matched, checking fallback intents...tytfyfhf"
    );
    const fallback = async (): Promise<string | null> => {
      ...
      const resolveAddressContext = async () => {
        ...
        const mentionsConnected =
          /(connected|my wallet|metamask|my eoa|connected eoa|wallet address)/i.test(
            lastUser!.content
          );
        const mentionsServer =
          /(server eoa|agent eoa|agent key|server wallet)/i.test(
            lastUser!.content
          );
        const mentionsSmart = /(smart account|smart|gasless)/i.test(
          lastUser!.content
        );
        ...
        if (mentionsEOAOnly) {
          ...
          return {
            target: clientEOA || eoa,
            label: clientEOA ? "Connected EOA" : "Server EOA",
            missing: !clientEOA && false,
          };
        }
      ...
      if (/\b(price|prices?|market|market data|top|tokens?)\b/.test(text)) {
        ...
        const symMatch = (lastUser!.content || "").match(
          /(?:price(?:\s+of)?\s+)?([a-z0-9]{2,10})/i
        );
        ...
        if (/\b(market|top|coins?|cryptocurrencies|tokens?)\b/.test(text)) {
          try {
            ...
            const nMatch =
              text.match(/top\s+(\d{1,2})/) ||
              text.match(/show\s*(?:me)?\s*(\d{1,2})/);
            const n = Math.min(
              50,
              Math.max(1, nMatch ? parseInt(nMatch[1], 10) : 5)
            );
            ...
            try {
              ...
              const topCoins = (Array.isArray(marketData) ? marketData : [])
                ...
                .map(
                  (coin: any) =>
                    `${coin.symbol}: $${Number(coin.price || 0).toFixed(4)}`
                )
                .join(", ");
              if (topCoins) {
            ...
            return `Couldn't fetch market data: ${
      ...
      if (/\b(gas|gas price|gas estimate|fees?)\b/.test(text)) {
        try {
          ...
          try {
            ...
            return `Couldn't fetch gas estimate: ${
              e instanceof Error ? e.message : "Unknown error"
            }`;
          }
        }
      ...
      // Portfolio overview
      if (
        /\b(portfolio|portfolio overview|total value|net worth)\b/.test(text)
      ) {
        try {
          ...
          const assets = portfolio.assets
            .map((asset: any) => {
              ...
              return `${asset.symbol}: ${formattedBalance} ($${fmtUSD(
                asset.valueUSD
              )})`;
            })
            .join("\n");
          ...
          return `Couldn't fetch portfolio: ${
            e instanceof Error ? e.message : "Unknown error"
          }`;
        }
      }
      ...
      if (/\b(transactions?|history|recent|tx)\b/.test(text)) {
        try {
          ...
          const recent = txs
            ...
            .map(
              (tx: any) =>
                `${tx.status === "success" ? "✅" : "❌"} ${tx.hash.slice(
                  0,
                  8
                )}...${tx.hash.slice(-6)}: ${tx.value} ${nativeSym}`
            )
            .join("\n");
          ...
          return `Couldn't fetch transaction history: ${
            e instanceof Error ? e.message : "Unknown error"
          }`;
        }
      }
      ...
      if (tr) {
        ...
        try {
          ...
          if (symbol.toUpperCase() === "AVAX") {
            ...
            await publicClient.waitForTransactionReceipt({ hash: txHash });
            return `✅ Successfully transferred ${amount} AVAX to ${to.slice(
              0,
              8
            )}...${to.slice(-6)}\n\nTransaction: ${txHash}`;
          } else {
            ...
            try {
              const { hash } = await smartTransfer({
                tokenAddress:
                  token.address === "AVAX" ? undefined : (token.address as any),
                amount,
                destination: to,
                wait: true,
              });
              return `✅ Successfully transferred ${amount} ${symbol} to ${to.slice(
                0,
                8
              )}...${to.slice(-6)}\n\nTransaction: ${hash}`;
            } catch (agentError) {
        ...
        try {
          ...
          for (const text of transferTexts) {
            const match = text.match(
              /(\d+(?:\.\d+)?)\s*([A-Za-z]{2,6})?\s*(?:to|=>)\s*(0x[a-fA-F0-9]{40})/
            );
            if (match) {
              ...
              const info = await getChainInfo();
              const token = symbol
                ? resolveTokenBySymbol(symbol, info.chainId)
                : null;
              transfers.push({
                destination: destination as `0x${string}`,
                amount,
                tokenAddress:
                  token && token.address === "AVAX"
                    ? undefined
      ...
      const scheduledTransferRe =
        /schedule\s+transfer\s+(\d+(?:\.\d+)?)\s*([A-Za-z]{2,6})?\s*(?:to|=>)\s*(0x[a-fA-F0-9]{40})\s+(?:for|at)\s+(.+)/i;
      ...
      if (scheduledMatch) {
        try {
          ...
          return `Scheduled transfer set for ${scheduleDate.toLocaleString()}. Amount: ${amount} ${
            symbol || "AVAX"
      ...
      const priorityTransferRe =
        /(urgent|fast|cheap|economy)\s+transfer\s+(\d+(?:\.\d+)?)\s*([A-Za-z]{2,6})?\s*(?:to|=>)\s*(0x[a-fA-F0-9]{40})/;
      ...
      if (priorityMatch) {
        ...
        try {
          ...
          if (priority === "cheap" || priority === "economy")
            routing = "cheapest";
          else if (priority === "urgent" || priority === "fast")
            routing = "fastest";
          ...
          } transfer submitted! Hash: ${result.hash}\nRouting: ${routing}`;
        } catch (e: any) {
          return `${
            priority.charAt(0).toUpperCase() + priority.slice(1)
      ...
      const smartTransferRe =
        /smart\s+transfer\s+(\d+(?:\.\d+)?)\s*([A-Za-z]{2,6})?\s*(?:to|=>)\s*(0x[a-fA-F0-9]{40})/;
      ...
      if (smartMatch) {
        try {
          ...
          return `Smart transfer executed! Hash: ${
            result.hash
      ...
      const swapRe =
        /swap\s+(\d+(?:\.\d+)?)\s*([A-Za-z]{2,6})\s*(?:to|for|->)\s*([A-Za-z]{2,6})/;
      ...
      if (sw) {
        ...
        try {
          try {
            ...
            console.log(
              "customSwap failed, falling back to smartSwap:",
              customErr?.message || customErr
            );
            try {
              const legacy = await smartSwap({
                tokenInSymbol: fromSym,
                tokenOutSymbol: toSym,
                amount,
                slippage: 0.5,
              ...
              try {
                ...
                const tokenInAddress =
                  inToken && inToken.address !== "AVAX"
                    ? (inToken.address as Address)
                    : undefined;
                const tokenOutAddress =
                  outToken && outToken.address !== "AVAX"
                    ? (outToken.address as Address)
                    : undefined;
                ...
                return `❌ Swap failed. customSwap: ${
                  customErr?.message || String(customErr)
                }; smartSwap: ${
                  legacyErr?.message || String(legacyErr)
                }; resolve: ${resolveErr?.message || String(resolveErr)}`;
              }
            }
          }
        } catch (e: any) {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "LLM not configured. Set OPENROUTER_API_KEY or OPENAI_API_KEY, or use simple commands: 'address', 'balance', 'transfer 0.01 to 0x..', 'swap 5 USDC to WETH'.",
        },
        { status: 429 }
      );
    }
    ...
    try {
      ...
      const last =
        Array.isArray(outMsgs) && outMsgs.length
          ? outMsgs[outMsgs.length - 1]
          : undefined;
      const content =
        last && typeof (last as any).content === "string"
          ? (last as any).content
          : (last?.content as any)?.toString?.() || "";
      return NextResponse.json({
        ok: true,
        content,
        threadId: (config as any).configurable.thread_id,
