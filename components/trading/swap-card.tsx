...
import {
  useTradeableAssets,
  AssetInfo,
} from "../../hooks/use-tradeable-assets";
...
export function SwapCard({ onPairChange }: SwapCardProps) {
  ...
  const [activeTab, setActiveTab] = useState<"swap" | "limit" | "buy" | "sell">(
    "swap"
  );
  ...
  const fetchQuote = async () => {
    ...
    try {
      ...
      const slippageBps = Number.isFinite(slippagePercent)
        ? Math.max(0, Math.round(slippagePercent * 100))
        : 50;
      ...
      if (data.bestRoute) {
        ...
        setAvailableRoutes(
          Array.isArray(data.routes) ? data.routes : [bestRoute]
        );
      } else {
        setToAmount("0");
        setRouteData(null);
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      const accountAddress = walletClient.account?.address as
        | Address
        | undefined;
      if (!accountAddress) {
        throw new Error(
          "Active account not detected. Please reconnect your wallet."
        );
      }

      if (walletClient.chain?.id && walletClient.chain.id !== CHAIN_ID) {
        throw new Error(
          "Please switch your wallet to the configured Celo network to swap."
        );
      }

      if (!ROUTER_ADDRESS || ROUTER_ADDRESS === ZERO_ADDRESS) {
        throw new Error(
          "Router address not configured. Set NEXT_PUBLIC_AMM_ROUTER in the environment."
        );
      }

      if (fromToken.address === "CELO") {
        throw new Error(
          "Native CELO swaps are not supported yet. Please wrap CELO to cCELO first."
        );
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || errorData.error || "Failed to prepare swap"
        );
      }
        ...
        toast({
          ...
          description: (
            <div className="mt-2 space-y-2 text-sm">
              ...
              <div>
                Input: {fromAmount} {fromToken.unitName}
              </div>
              <div>
                Output: ~{expectedOutFormatted} {toToken.unitName}
              </div>
              <div className="text-xs text-muted-foreground">
                Min received (after slippage): {minOutFormatted}{" "}
                {toToken.unitName}
              </div>
              <a
                href={`${
                  EXPLORER_MAP[CHAIN_ID] || DEFAULT_EXPLORER
                }/tx/${demoHash}`}
                target="_blank"
      ...
      const expectedOutFormatted = formatUnits(
        BigInt(payload.details.expectedOut),
        toToken.decimals
      );
      const minOutFormatted = formatUnits(
        BigInt(payload.details.minOut),
        toToken.decimals
      );
      ...
      toast({
        ...
        description: (
          <div className="mt-2 space-y-2 text-sm">
            ...
            <div>
              Swapped: {fromAmount} {fromToken.unitName}
            </div>
            <div>
              Received: ~{expectedOutFormatted} {toToken.unitName}
            </div>
            <div className="text-xs text-muted-foreground">
              Min received (after slippage): {minOutFormatted}{" "}
              {toToken.unitName}
                  ...
                  <div className="space-y-2 flex flex-col grow text-muted-foreground">
                    ...
                    <span className="h-5 inline-flex items-center whitespace-nowrap text-sm">
                      ...
                      {fromAmount && fromToken
                        ? (parseFloat(fromAmount) * 0.18).toFixed(2)
                        : "0.00"}
                    </span>
                  </div>
                ...
                <div className="flex w-full gap-2 px-3 py-3 min-h-24 items-center justify-between group transition-all duration-300 bg-muted/50 rounded-lg border border-border focus-within:border-primary focus-within:bg-background h-[7.5rem]">
                  <div className="space-y-2 flex flex-col grow text-muted-foreground">
                    ...
                    <span className="h-5 inline-flex items-center whitespace-nowrap text-sm">
                      $
                      {toAmount && toToken
                        ? (parseFloat(toAmount) * 1.0).toFixed(2)
                        : "0.00"}
                    </span>
                  </div>
                  ...
                  <div className="w-full p-3 bg-muted/50 rounded-lg space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        1 {fromToken.unitName} ={" "}
                        {(
                          parseFloat(toAmount || "0") /
                          Math.max(parseFloat(fromAmount || "0"), 1e-12)
                        ).toFixed(6)}{" "}
                        {toToken.unitName}
                      </span>
                    </div>
                    ...
                    {routeData.pools.length > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {Array.from(
                            new Set(routeData.pools.map((pool) => pool.dex))
                          ).join(" → ")}
                        </span>
                      </div>
                    )}
                    ...
                    {routeData.priceImpactPercent !== null && (
                      <div className="flex justify-between">
                        ...
                        <span
                          className={cn(
                            "font-medium",
                            routeData.priceImpactPercent > 5
                              ? "text-destructive"
                              : ""
                          )}
                        >
