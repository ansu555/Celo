...
export default function ChatBubble({
  ...
  const renderWithAddresses = (segment: string, keyPrefix: string) => {
    ...
    if (lastIndex < segment.length) {
      nodes.push(
        ...renderTextWithUrls(segment.slice(lastIndex), `${keyPrefix}-tail`)
      );
    }
  ...
  const presets: { label: string; prompt: string }[] = [
    ...
    {
      label: "Transfer",
      prompt: `transfer 0.01 ${nativeSymbol} to AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAY5HFKQ`,
    },
  ];
  ...
  return (
    <div className={wrapperClass}>
      ...
      >
        <span
          className="pointer-events-none absolute inset-0 rounded-full bg-gradient-to-br from-red-500/20 via-transparent to-red-500/50 blur-lg"
          aria-hidden
        />
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={
              isFooter ? { opacity: 1, y: 0, scale: 1 } : { x: 0, opacity: 1 }
            }
            exit={
              isFooter
                ? { opacity: 0, y: 16, scale: 0.98 }
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 border-b border-slate-200/60 px-4 py-3 dark:border-white/10">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setOpen(false)}
                  className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-white/5 dark:hover:text-white"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="border-b border-slate-200/60 px-3 py-2 text-xs text-slate-600 dark:border-white/10 dark:text-slate-300">
                <div className="mb-1 flex items-center justify-between">
                  <span className="font-semibold text-slate-900 dark:text-slate-200">
                    Keep this in min(guideline generated by cursor)
                  </span>
                  <button
                ...
                {showRules && (
                  <div className="relative rounded-lg border border-red-300/30 bg-red-50/80 p-2 dark:border-[#F3C623]/30 dark:bg-[#F3C623]/10">
                    ...
                    <div className="grid max-h-48 gap-2 overflow-y-auto pr-2 pl-2">
                      <div>
                        <div className="mb-1 font-medium text-slate-900 dark:text-slate-200">
                          Supported actions
                        </div>
