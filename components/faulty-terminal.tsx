...
export default function FaultyTerminal({
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const x = (e as MouseEvent & any).clientX - rect.left;
    const nx = x / rect.width;
    const y = 1 - ((e as MouseEvent & any).clientY - rect.top) / rect.height;
    mouseRef.current = { x: nx, y };
  }, []);
  ...
  useEffect(() => {
    ...
    const program = new Program(gl as any, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
          value: new Color((gl.canvas as any).width, (gl.canvas as any).height, (gl.canvas as any).width / (gl.canvas as any).height)
        },
        uScale: { value: scale },
    ...
    rafRef.current = requestAnimationFrame(update as any);
    ctn.appendChild((gl.canvas as any));
